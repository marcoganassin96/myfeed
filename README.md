...
MDG exposes an interactive API explorer generated by [NelmioApiDocBundle](docs/php-learning-path/05-nelmio-api-doc-bundle.md) from PHP attributes on each controller. The spec is code-first — no separate build step, always in sync with the real routes.

28 endpoints across three access scopes (ADR-012):

| Scope | Auth | Swagger UI | Endpoints |
|---|---|---|---|
| 🌐 **Public** | none | `/api/doc/public` | `GET /master-data/topics`, `GET /master-data/topics/{id}` |
| 👤 **User** | `X-User-Id` header | `/api/doc/user` | news-events (2), newsletters (2), subscriptions (3), interactions (1), deep-dive (2) |
| 🔒 **Admin** | `X-Admin-Token` header | `/api/doc/admin` | topics (3), news-events (5), newsletters (5), subscriptions (2), interactions (1) |

`/api/doc` shows all 28 endpoints. Each scoped page pre-populates the correct auth header in the Authorize dialog.

**User endpoints:**

| Method | Path |
|---|---|
| GET | /master-data/news-events |
| GET | /master-data/news-events/{id} |
| GET | /master-data/newsletters |
| GET | /master-data/newsletters/{id} |
| GET | /master-data/subscriptions |
| POST | /master-data/subscriptions |
| DELETE | /master-data/subscriptions/{topicId} |
| POST | /master-data/interactions |
| GET | /master-data/deep-dive/{eventId} |
| POST | /master-data/deep-dive/{eventId} |

**Admin endpoints:**

| Method | Path |
|---|---|
| POST | /master-data/admin/topics |
| PUT | /master-data/admin/topics/{id} |
| DELETE | /master-data/admin/topics/{id} |
| GET | /master-data/admin/news-events |
| GET | /master-data/admin/news-events/{id} |
| POST | /master-data/admin/news-events |
| PUT | /master-data/admin/news-events/{id} |
| DELETE | /master-data/admin/news-events/{id} |
| GET | /master-data/admin/newsletters |
| GET | /master-data/admin/newsletters/{id} |
| POST | /master-data/admin/newsletters |
| PUT | /master-data/admin/newsletters/{id} |
| DELETE | /master-data/admin/newsletters/{id} |
| GET | /master-data/admin/subscriptions |
| DELETE | /master-data/admin/subscriptions/{userId}/{topicId} |
| GET | /master-data/admin/interactions |
...
mdg/                       PHP 8.4 / Symfony 7 — master data gateway
  src/
    Controller/            Public: TopicController
                           User:   NewsletterController, NewsEventController,
                                   SubscriptionController, InteractionController, DeepDiveController
                           Admin:  AdminNewsletterController, AdminNewsEventController,
                                   AdminTopicController, AdminSubscriptionController,
                                   AdminInteractionController
    Service/               NewsletterService, NewsEventService, TopicService,
    ...
    Entity/                Newsletter, NewsEvent, Topic, Subscription, Interaction, DeepDive
    Cache/                 CacheService, PredisAdapter, RedisClientInterface
    EventListener/         AdminTokenListener (X-Admin-Token on /admin/* routes)
  tests/
    Controller/            PHPUnit controller tests
    Service/               PHPUnit service tests
    Cache/                 PHPUnit cache tests
    Smoke/                 EndpointSmokeTest (live HTTP against running server)
